...
import { getPostgresPokemon } from '@/server/data/postgres';
...
export async function GET(request: Request, { params }: { params: Promise<{ speciesId: string }> }) {
  ...
  const pokemon = context.runtime === 'postgresql'
    ? await getPostgresPokemon(context.postgresContext!, formId ?? speciesId, speciesId)
    : formId
