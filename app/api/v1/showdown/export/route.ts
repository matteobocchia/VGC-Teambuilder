...
export async function POST(request: Request) {
  ...
  const context = resolveContext(new Request(`https://vgc.local/api/v1/showdown/export?formatId=${encodeURIComponent(formatId)}&dataReleaseId=${encodeURIComponent(releaseId)}`));
